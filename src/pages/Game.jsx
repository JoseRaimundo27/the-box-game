...
const Game = () => {
  ...
  // 2. REGRAS DA ESTAÇÃO DINÂMICAS
  const specs = {
    station_A: {
      stockNeeded: config.stockNeeded.A,
      wipNeeded: 0,
      nextWip: "ab",
    },
    station_B: {
      stockNeeded: config.stockNeeded.B,
      wipNeeded: 1,
      nextWip: "bc",
      prevWip: "ab",
    },
    station_C: {
      stockNeeded: config.stockNeeded.C,
      wipNeeded: 1,
      nextWip: "cd",
      prevWip: "bc",
    },
    station_D: {
      stockNeeded: config.stockNeeded.D,
      wipNeeded: 1,
      nextWip: "de",
      prevWip: "cd",
    },
    station_E: {
      stockNeeded: config.stockNeeded.E,
      wipNeeded: 1,
      nextWip: "finish",
      prevWip: "de",
    },
  ...
  const WIP_STAGE_PRICES = {
    ...
    bc:
      config.stockNeeded.A * ITEM_PRICES.A +
      config.stockNeeded.B * ITEM_PRICES.B,
    cd:
      config.stockNeeded.A * ITEM_PRICES.A +
      config.stockNeeded.B * ITEM_PRICES.B +
      config.stockNeeded.C * ITEM_PRICES.C,
    de:
      config.stockNeeded.A * ITEM_PRICES.A +
      config.stockNeeded.B * ITEM_PRICES.B +
      config.stockNeeded.C * ITEM_PRICES.C +
      config.stockNeeded.D * ITEM_PRICES.D,
  };
  ...
  const getGabaritoComposition = (letter) => {
    ...
    for (let i = 0; i < config.stockNeeded[letter]; i++)
      comp.push(colors[letter]);
    return comp;
  };
  ...
  useEffect(() => {
    ...
    const prodExists = roomData?.rounds?.[currentRnd]?.production;
    ...
    if (roomData && !prodExists && currentRoom) {
      const initialProduction = {
        ...
        workAreas: {
          A: { stockItems: 0, wipItems: 0 },
          B: { stockItems: 0, wipItems: 0 },
          C: { stockItems: 0, wipItems: 0 },
          D: { stockItems: 0, wipItems: 0 },
          E: { stockItems: 0, wipItems: 0 },
        },
      };

      const prodRef = ref(
        db,
        `rooms/${currentRoom}/rounds/${currentRnd}/production`,
      );
      import("firebase/database").then(({ set }) => {
        set(prodRef, initialProduction);
      });
    }
  ...
  const handleAction = (type) => {
    ...
    runTransaction(prodRef, (current) => {
      ...
      if (type === "STOCK") {
        if (
          current.workAreas[sLetter].stockItems < mySpec.stockNeeded &&
          current.stocks[sLetter] > 0
        ) {
      ...
      if (type === "WIP") {
        if (
          current.wips[mySpec.prevWip] > 0 &&
          current.workAreas[sLetter].wipItems === 0
        ) {
    ...
    <div className="game-screen">
      ...
      >
        <div className="production-progress">
          <strong>
            {t("game.progress")} (Round {currentRound})
          </strong>{" "}
          {prod.finished_total} / {META_PRODUCAO}
          {/* TIMER REGRESSIVO */}
          {timeLeft !== null && (
            <span
              style={{
